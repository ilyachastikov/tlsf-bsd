...
| `TLSF_MSVC_CUSTOM_LZCNT_GUARD` | off | With `TLSF_MSVC_MODERN_INTRINSICS` on, disables MSVC's automatic x86/x64 LZCNT guard, including the no-CRT build error; define only when the host performs its own LZCNT hardware validation |
